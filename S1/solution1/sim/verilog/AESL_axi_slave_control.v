...
//------------------------Parameter----------------------
`define TV_IN_inPtr_V "../tv/cdatafile/c.computeS1.autotvin_inPtr_V.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter inPtr_V_DEPTH = 1;
reg [31 : 0] inPtr_V_OPERATE_DEPTH = 1;
parameter inPtr_V_c_bitwidth = 32;
parameter inPtr_V_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [inPtr_V_c_bitwidth - 1 : 0] reg_inPtr_V;
reg inPtr_V_write_data_finish;
...
//write inPtr_V reg
reg [31 : 0] write_inPtr_V_count = 0;
reg write_inPtr_V_run_flag = 0;
reg write_one_inPtr_V_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & inPtr_V_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        inPtr_V_write_data_finish <= 0;
        write_inPtr_V_run_flag <= 0; 
        write_inPtr_V_count = 0;
        count_operate_depth_by_bitwidth_and_depth (inPtr_V_c_bitwidth, inPtr_V_DEPTH, inPtr_V_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_inPtr_V_run_flag <= 1; 
            write_inPtr_V_count = 0;
        end
        if (write_one_inPtr_V_data_done === 1) begin
            write_inPtr_V_count = write_inPtr_V_count + 1;
            if (write_inPtr_V_count == inPtr_V_OPERATE_DEPTH) begin
                write_inPtr_V_run_flag <= 0; 
                inPtr_V_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_inPtr_V
    integer write_inPtr_V_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] inPtr_V_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = inPtr_V_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_inPtr_V_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write inPtr_V data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (inPtr_V_c_bitwidth < 32) begin
                        inPtr_V_data_tmp_reg = reg_inPtr_V;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < inPtr_V_c_bitwidth) begin
                                inPtr_V_data_tmp_reg[j] = reg_inPtr_V[i*32 + j];
                            end
                            else begin
                                inPtr_V_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (inPtr_V_data_in_addr + write_inPtr_V_count * four_byte_num * 4 + i * 4, inPtr_V_data_tmp_reg, write_inPtr_V_resp);
                end
                process_busy = 0;
                write_one_inPtr_V_data_done <= 1;
                @(posedge clk);
                write_one_inPtr_V_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
